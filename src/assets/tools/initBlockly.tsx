...
defineBlocksWithJsonArray([
    ...
    javascriptGenerator['renommage'] = function(block: any) {
        ...
        const champsTuples = value_champs.split(',');
    ...
    javascriptGenerator['selection'] = function(block: any) {
        ...
        const conditionsString = value_champs.split(' && ');
        ...
        functionCheck += '; }';
        let code = statements_elements + '.selectRowsWithCheck(' + functionCheck + ')';
        return code;
    };


    javascriptGenerator['projection'] = function(block: any) {
        const value_champs = javascriptGenerator.statementToCode(block, 'Champs', javascriptGenerator.ORDER_ADDITION);
        const statements_elements = javascriptGenerator.statementToCode(block, 'ensemble', javascriptGenerator.ORDER_ADDITION);
        let code = statements_elements;
        const champs = value_champs.split(',');
        code += '.selectColumns(' + JSON.stringify(champs) + ')';
        return code;
    };
    javascriptGenerator['ensemble'] = function(block: any) {
        const textValue = javascriptGenerator.statementToCode(block, 'name', javascriptGenerator.ORDER_ADDITION);
        const code = 'WorkspaceRelations.getTableByName("' + textValue + '")';
        return code;
    };
    ...
    javascriptGenerator['difference'] = function(block: any) {
        const statements_elements1 = javascriptGenerator.statementToCode(block, 'ensemble1');
        const statements_elements2 = javascriptGenerator.statementToCode(block, 'ensemble2');
        let code = statements_elements1;
        code += '.difference(' + statements_elements2 + ')';
        return code;
    }
    javascriptGenerator['debut'] = function(block: any) {
        const statements_elements = javascriptGenerator.statementToCode(block, 'suivant');
        return statements_elements;
    }
    ...
    sampleGenerator['renommage'] = function(block: any) {
        const value_champs = sampleGenerator.statementToCode(block, 'Champs');
        const statements_elements = sampleGenerator.statementToCode(block, 'ensemble');
        const code = '\\rho_{' + value_champs + ' }(' + statements_elements + ')';
        return code;
    };
    sampleGenerator['selection'] = function(block: any) {
        const value_champs = sampleGenerator.statementToCode(block, 'Champs');
        const statements_elements = sampleGenerator.statementToCode(block, 'ensemble');
        const code = '\\sigma_{' + value_champs + '}(' + statements_elements + ')';
        return code;
    };
    ...
    sampleGenerator['projection'] = function(block: any) {
        const value_champs = sampleGenerator.statementToCode(block, 'Champs');
        const statements_elements = sampleGenerator.statementToCode(block, 'ensemble');
        const code = '\\pi_{' + value_champs + ' }(' + statements_elements + ')';
        return code;
    };

    sampleGenerator['ensemble'] = function(block: any) {
        const value_champs = sampleGenerator.statementToCode(block, 'name');
        const code = value_champs;
        return code;
    };
    ...
    sampleGenerator['difference'] = function(block: any) {
        const statements_elements1 = sampleGenerator.statementToCode(block, 'ensemble1');
        const statements_elements2 = sampleGenerator.statementToCode(block, 'ensemble2');
        const code = '(' + statements_elements1 + ') \\\\ (' + statements_elements2 + ')';
        return code;
    }
    ...
    sampleGenerator['intersection'] = function(block: any) {
        const statements_elements1 = sampleGenerator.statementToCode(block, 'ensemble1');
        const statements_elements2 = sampleGenerator.statementToCode(block, 'ensemble2');
        const code = '(' + statements_elements1 + ') \\cup (' + statements_elements2 + ')';
        return code;
    }
    ...
    sampleGenerator['union'] = function(block: any) {
        const statements_elements1 = sampleGenerator.statementToCode(block, 'ensemble1');
        const statements_elements2 = sampleGenerator.statementToCode(block, 'ensemble2');
        const code = '(' + statements_elements1 + ') \\cap (' + statements_elements2 + ')';
        return code;
    }
    ...
    sampleGenerator['produit'] = function(block: any) {
        const statements_elements1 = sampleGenerator.statementToCode(block, 'ensemble1');
        const statements_elements2 = sampleGenerator.statementToCode(block, 'ensemble2');
        const code = '(' + statements_elements1 + ') \\times [' + statements_elements2 + ']';
        return code;
    }

    sampleGenerator['debut'] = function(block: any) {
        const statements_elements = sampleGenerator.statementToCode(block, 'suivant');
    ...
    sampleGenerator['jointure'] = function(block: any) {
        ...
        const statements_elements1 = sampleGenerator.statementToCode(block, 'ensemble1');
        const statements_elements2 = sampleGenerator.statementToCode(block, 'ensemble2');
        const code = '(' + statements_elements1 + ') \\bowtie_{' + value_champs + '} (' + statements_elements2 + ')';
        return code;
    };
    sampleGenerator['division'] = function(block: any) {
        const statements_elements1 = sampleGenerator.statementToCode(block, 'ensemble1');
        const statements_elements2 = sampleGenerator.statementToCode(block, 'ensemble2');
        const code = '(' + statements_elements1 + ') \\div (' + statements_elements2 + ')';
        return code;
    };
    /* -------------------------------------------------------------------------- */
